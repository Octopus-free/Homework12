        ...
        for page_number in range(0, 101):
            ...
            for vacancy in hh_response['items']:
                for remaining in range(3, 0, -1):
                    sys.stdout.write("\r")
                    sys.stdout.write("{} сек. сенкудная задержка из-за ограничений "
                                     "hh.ru, загружаются вакансии со страницы {}.".format(remaining, page_number+1))
                    sys.stdout.flush()
                    time.sleep(1)

                # скачиваем информацию о вакансии и сохраняем ее в словарь
                current_response = requests.get(vacancy['url']).json()
                vacancies_dict[vacancy['id']] = {'url': vacancy['url'],
